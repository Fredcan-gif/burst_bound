...
func _input(event):
	if event.is_action_pressed("ui_cancel") and GameManager.game_started and not GameManager.player_dead and not GameManager.is_boss_intro and not GameManager.is_tutorial:
